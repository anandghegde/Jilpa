...
**Context engine.** Contexts come from config. A pin names a context or an ad hoc project folder with expiry `untilChanged`, `until(date)` or `untilQuit`. Expiry uses one scheduled timer, re-armed on wake; `untilQuit` pins are cleared at launch. Precedence is pin, then (P1) sensed switching, then the manually selected context, then none.
...
- **Settings (S10).** SwiftUI, five panes. Every option is declared once in a `SettingDescriptor` registry that drives both the pane and the search index, so "every option searchable" holds by construction.
- **Onboarding and health (S11).** Onboarding runs the live demo against `JilpaDemo.app`. A short timer while the onboarding window waits for the Accessibility grant is the only polling in the app. `HealthCenter` evaluates permissions, config status, compatibility data age and per-app support, and emits one notice per state change, never repeats.
...
21. **Pinning a recent has no gate row.** D5 asks for pinned recents and `dest_stat.pinned` exists, but every store write takes a `Cleared` record and no operation fits a pin: it is the user's own act like a favorite, it is made from the menu with no dialog and often no app in front, and it marks a row that only exists because activity was learned. `learn` needs a known app and a recording dialog, and `keepConfiguredIdentity` is for configuration. Proposed: a `pinRecent` operation, allowed wherever the recents menu is allowed, refused in private mode because the list is not shown there, and subject to exclusions like any record. Until the owner decides, the read side honors `pinned` and nothing writes it. A second open point is whether a pin made in one app's list pins the place everywhere; the read model treats a place as pinned in a list when any counter in that list's scope is.
